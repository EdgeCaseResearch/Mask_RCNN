...
class ECRKerasModel(DifferentiableModel):
    ...
    def __init__(
            ...
            output_label_idx=None):

        super(ECRKerasModel, self).__init__(bounds=bounds,
        ...
        assert not (isinstance(images_input, list) and input_image_idx is None), "If there are multiple inputs, input_image_idx must be specified."
        if isinstance(images_input, list):
            images_input = images_input[input_image_idx]

        label_input = K.placeholder(shape=(1,))
        
        predictions = model.output
        assert not (isinstance(predictions, list) and output_label_idx is None), "If there are multiple outputs, output_label_idx must be specified."
        if isinstance(predictions, list):
            predictions = predictions[output_label_idx]
    ...
    def predict_on_inputs(self, inputs):
        return self.keras_model.predict(inputs)
